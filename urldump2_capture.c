...
void
process_http_request(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	struct HttpContext *http_ctx = (struct HttpContext *)malloc(sizeof(struct HttpContext));
	if(http_ctx == NULL){
